...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"drganiev");
    if (!NT_SUCCESS(status))
    {
...
--*/
{
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) { //Шифрование
            ...
            //здесь исп. сторонней библиотеки
            if (Data->Iopb->Parameters.Write.WriteBuffer) {
                ...
                unsigned char k[] = "0JPQsNC90LjQtdCy";    //16 битный ключ
